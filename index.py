...
def attribute():
    if request.method == 'POST':
        pimg = request.files['video']
        p1=pimg.filename
        p1name=secure_filename(pimg.filename)
        pimg.save(os.path.join(app.config['UPLOAD_FOLDER'], p1name))
        #mydict=request.form
        ...
        while True:
            ...
            if ret:  # if there is a frame continue with code
                ...
                #f1 = open(p1+cdate+".txt", "a")
                f1 = open(p1name+cdate+".txt", "a")
                
                
                
                
                f = open("output.txt", "w")
                f.write("Cars in Area : "+ str(currentcars) + "\n")
                f1.write("Cars in Area : "+ str(currentcars) + "\n")
        
        
                cv2.putText(image, "Cars Crossed Up: " + str(carscrossedup), (0, 30), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 170, 0),
                            1)
                
                f.write("Cars Crossed Up : "+ str(carscrossedup) + "\n")
                f1.write("Cars Crossed Up : "+ str(carscrossedup) + "\n")
        
                cv2.putText(image, "Cars Crossed Down: " + str(carscrosseddown), (0, 45), cv2.FONT_HERSHEY_SIMPLEX, .5,
                            (0, 170, 0), 1)
                
                f.write("Cars Crossed Down : "+ str(carscrosseddown) + "\n")
                f1.write("Cars Crossed Down : "+ str(carscrosseddown) + "\n")

                cv2.putText(image, "Total Cars Detected: " + str(len(carids)), (0, 60), cv2.FONT_HERSHEY_SIMPLEX, .5,
                            (0, 170, 0), 1)
                
                f.write("Total Cars Detected : "+ str(len(carids)) + "\n")
                f1.write("Total Cars Detected : "+ str(len(carids)) + "\n")
